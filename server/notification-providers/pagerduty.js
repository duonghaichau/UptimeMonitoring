    ...
    async send(notification, msg, monitorJSON = null, heartbeatJSON = null) {
        try {
            if (heartbeatJSON == null) {
                const title = "Uptime ZaloPay Alert";
                const monitor = {
                    type: "ping",
                    url: "Uptime ZaloPay Test Button",
                };
                return this.postNotification(notification, title, msg, monitor);
            }

            if (heartbeatJSON.status === UP) {
                const title = "Uptime ZaloPay Monitor ✅ Up";
            ...
            if (heartbeatJSON.status === DOWN) {
                const title = "Uptime ZaloPay Monitor 🔴 Down";
    ...
    async postNotification(notification, title, body, monitorInfo, eventAction = "trigger") {
        ...
        const options = {
            ...
            data: {
                ...
                dedup_key: "Uptime ZaloPay/" + monitorInfo.id,
            }
        };

        const baseURL = await setting("primaryBaseURL");
        if (baseURL && monitorInfo) {
            options.client = "Uptime ZaloPay";
